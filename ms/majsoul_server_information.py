...
def get_endpoint_and_version():
    ...
    try:
        ...
        resversion = get_majsoul_resource('resversion{}.json'.format(version['version']))
        protobuf_version = resversion['res']["res/proto/liqi.json"]['prefix']
        protobuf_schema = get_majsoul_resource('{}/res/proto/liqi.json'.format(protobuf_version))
        config = get_majsoul_resource('{}/config.json'.format(resversion['res']['config.json']['prefix']))
    ...
    for attempt in range(len(request_game_server_endpoint_list) * MAX_ATTEMPT_PER_SERVER):
        request_game_server_endpoint = request_game_server_endpoint_list[attempt // MAX_ATTEMPT_PER_SERVER]['url']
